...
export async function POST(request: NextRequest) {
  ...
  if (!service) return Response.json({ id: crypto.randomUUID(), status: "sent", demo: true }, { status: 201 })
  const { location, accuracyMeters, emergencyTypes } = parsed.data
  const locationValue = `SRID=4326;POINT(${location[0]} ${location[1]})`
  const { data, error } = await service.from("sos_alerts").insert({ user_id: authData.user?.id || null, location: locationValue, accuracy_meters: accuracyMeters, emergency_types: emergencyTypes, status: "sent" }).select("id,status").single()
