...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await service.from("reports").insert({ user_id: authData.user?.id || null, geom: line, start_point: startPoint, end_point: endPoint, length_meters: report.lengthMeters, depth_level: report.depthLevel, photo_url: report.photoUrl || null, note: report.note || null, street_name: report.streetName, barangay: report.barangay, report_mode: report.mode }).select("id").single()
  if (error) return safeJsonError("Report could not be saved", 500)
