...
import { CircleUserRound, LifeBuoy, Map, Plus, Radio, ShieldCheck } from "lucide-react"
...
export function DesktopSidebar() {
  ...
  useEffect(() => { fetch("/api/admin", { cache: "no-store" }).then((r) => setIsAdmin(r.ok)).catch(() => setIsAdmin(false)) }, [])
  const items = isAdmin ? [...navigationItems, { href: "/admin", label: "Admin", shortLabel: "Admin", icon: ShieldCheck }] : navigationItems
  return (
...
export function MobileBottomNav() {
  ...
  const items = isAdmin ? [...navigationItems, { href: "/admin", label: "Admin", shortLabel: "Admin", icon: ShieldCheck }] : navigationItems
  return (
    <nav className={cn("safe-area-bottom glass fixed inset-x-0 bottom-0 z-50 grid border-t px-1 pt-2 lg:hidden", isAdmin ? "grid-cols-5" : "grid-cols-4")} aria-label="Main navigation">
      {items.map((item) => {
        const Icon = item.icon
        const active = pathname === item.href
