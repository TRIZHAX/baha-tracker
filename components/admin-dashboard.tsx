  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <section className="rounded-3xl border bg-[hsl(var(--card))] p-4 shadow-soft sm:p-5">
        ...
        <div className="overflow-x-auto rounded-2xl border"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-[hsl(var(--muted))] text-xs uppercase tracking-wide"><tr><th className="p-3">Report</th><th className="p-3">Reporter</th><th className="p-3">Area</th><th className="p-3">Depth</th><th className="p-3">Status</th><th className="p-3">Created</th><th className="p-3">Votes</th><th className="p-3">Actions</th></tr></thead><tbody>{loading ? <tr><td colSpan={8} className="p-6 text-center">Loading reports…</td></tr> : reports.length === 0 ? <tr><td colSpan={8} className="p-6 text-center text-[hsl(var(--muted-foreground))]">No reports found.</td></tr> : reports.map((report) => <tr key={report.id} className="border-t align-top"><td className="p-3"><div className="font-mono text-xs">{report.id.slice(0, 8)}…</div><div className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{report.report_mode}</div></td><td className="p-3">{report.reporter_email ?? "Guest / unknown"}</td><td className="p-3"><div className="font-semibold">{report.street_name}</div><div className="text-xs text-[hsl(var(--muted-foreground))]">{report.barangay}</div></td><td className="p-3 capitalize">{report.depth_level.replace("_", " ")}</td><td className="p-3"><select aria-label={`Update report ${report.id} status`} disabled={busyId === report.id} value={report.verification_status} onChange={(event) => void updateReport(report.id, event.target.value as VerificationStatus)} className="rounded-xl border bg-transparent px-2 py-2 font-semibold">{reportStatuses.map((status) => <option key={status} value={status}>{statusText[status]}</option>)}</select></td><td className="p-3 whitespace-nowrap"><div>{new Date(report.created_at).toLocaleString()}</div><div className="text-xs text-[hsl(var(--muted-foreground))]">expires {new Date(report.expires_at).toLocaleString()}</div></td><td className="p-3 whitespace-nowrap">↑ {report.upvotes} · ↓ {report.downvotes}</td><td className="p-3"><button type="button" disabled={busyId === report.id} onClick={() => void deleteReport(report.id)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-red-200 px-3 font-bold text-red-700 hover:bg-red-50 disabled:opacity-50 dark:border-red-900 dark:text-red-300 dark:hover:bg-red-950"><Trash2 className="h-4 w-4" />Delete</button></td></tr>)}</tbody></table></div>
      </section>
